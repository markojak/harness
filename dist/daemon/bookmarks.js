...
export function addBookmark(data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    stmt.run(id, data.sessionId, data.projectId || null, data.projectName || null, data.title, data.notes || "", JSON.stringify(data.tags || []), data.originalPrompt || null, now, now);
    return {
        id,
...
export function updateBookmark(id, data) {
    ...
    WHERE id = ?
  `);
    stmt.run(data.title ?? existing.title, data.notes ?? existing.notes, JSON.stringify(data.tags ?? existing.tags), now, id);
 ...
 */
export function removeBookmarkBySession(sessionId) {
 ...
 */
export function getBookmarkBySession(sessionId) {
 ...
 */
export function listBookmarksByProject(projectId) {
