...
function formatRepoLabel(session) {
...
function logSessionEvent(event) {
    ...
    switch (type) {
        case "created":
            console.log(`${colors.gray}${timestamp}${colors.reset} ${colors.green}[NEW]${colors.reset} ${colors.cyan}${session.sessionId.slice(0, 8)}${colors.reset} ${repo}`);
        ...
        case "updated":
            ...
            console.log(`${colors.gray}${timestamp}${colors.reset} ${colors.yellow}[CHG]${colors.reset} ${colors.cyan}${session.sessionId.slice(0, 8)}${colors.reset} ${repo} ${colors.dim}${prevStatusKey} → ${newStatusKey}${colors.reset}`);
            console.log(`  ${colors.bold}${cwdShort}${colors.reset} ${branch}`);
            console.log(`  ${status} | ${msgCount} msgs | ${lastActivity}`);
            break;
        case "deleted":
            console.log(`${colors.gray}${timestamp}${colors.reset} ${colors.blue}[DEL]${colors.reset} ${colors.cyan}${session.sessionId.slice(0, 8)}${colors.reset}`);
...
function logInitialSessions(sessions) {
    // Filter sessions
    ...
    for (const repoKey of sortedKeys) {
        ...
        for (const [cwd, cwdSessions] of byCwd) {
            ...
            for (const session of cwdSessions) {
                ...
                console.log(`${indent}${colors.cyan}${session.sessionId.slice(0, 8)}${colors.reset}${branch} ${status}`);
