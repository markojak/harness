 ...
 */
async function getKnownGitRoots() {
    const roots = new Set();
    try {
 ...
 */
async function commitExistsInRepo(hash, repoPath) {
    try {
 ...
 */
export async function findRepoForCommit(hash) {
    const roots = await getKnownGitRoots();
    // Check all repos in parallel
    const results = await Promise.all(roots.map(async (repoPath) => {
 ...
 * Get commit details
 */
export async function getCommitInfo(hash, repoPath) {
    try {
        // Get commit metadata
        const { stdout: meta } = await execAsync(`git -C ${JSON.stringify(repoPath)} log -1 --format="%H%n%s%n%an%n%aI" ${hash}`, { timeout: 5000 });
        const [fullHash, message, author, date] = meta.trim().split("\n");
        // Get changed files
        const { stdout: filesRaw } = await execAsync(`git -C ${JSON.stringify(repoPath)} show --name-only --format="" ${hash}`, { timeout: 5000 });
 ...
 */
export async function findSessionsForCommit(commitInfo) {
 ...
 */
export async function findCommit(hash) {
