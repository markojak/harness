...
function parseGitUrl(url) {
    ...
    const httpsMatch = url.match(/^https?:\/\/(?:www\.)?github\.com\/([^/]+)\/([^/\s]+?)(?:\.git)?$/i);
    if (httpsMatch) {
 ...
 */
async function _getCurrentBranchInternal(gitDir) {
    try {
 ...
 */
async function getOriginUrl(gitDir) {
    try {
 ...
 */
export async function getGitInfoCached(cwd) {
 ...
 */
async function getCurrentBranchFromDir(gitDir) {
    try {
        const headPath = join(gitDir, "HEAD");
        const headContent = await readFile(headPath, "utf-8");
        const trimmed = headContent.trim();
 ...
 */
export function clearGitCache(cwd) {
