 ...
 */
export function setOnPRUpdate(callback) {
    onPRUpdate = callback;
}
/**
 * Process queue tasks
...
const queue = fastq.promise(processTask, 2);
/**
 * Check if a branch has an associated PR
 */
async function checkPRForBranch(cwd, branch, sessionId) {
    ...
    try {
        ...
        const { stdout } = await execAsync(`gh pr list --head "${branch}" --json number,url,title,headRefName --limit 1`, { cwd });
        ...
        logError("PR", `Failed to check PR for ${branch}`, error);
        prCache.set(cacheKey, { pr: null, lastChecked: Date.now() });
    }
}
/**
 ...
 */
async function getCIStatus(cwd, prNumber) {
    try {
        const { stdout } = await execAsync(`gh pr checks ${prNumber} --json name,state,link`, { cwd });
        ...
        logError("PR", `Failed to get CI status for PR #${prNumber}`, error);
 ...
 */
async function checkCIStatus(cwd, prNumber, sessionId) {
    try {
        ...
        logError("PR", `Failed to check CI for PR #${prNumber}`, error);
    }
}
/**
 * Start active CI polling for a PR
 */
function startCIPolling(cwd, prNumber, sessionId) {
    // Don't start if already polling
    if (activeCIPolling.has(sessionId))
        return;
 ...
 */
function startIdleCIPolling(cwd, prNumber, sessionId) {
    stopCIPolling(sessionId);
    const interval = setInterval(() => {
        queue.push({ type: "check_ci", cwd, prNumber, sessionId });
 ...
 */
function stopCIPolling(sessionId) {
 ...
 */
export function queuePRCheck(cwd, branch, sessionId) {
    if (!branch)
        return;
 ...
 */
export function getCachedPR(cwd, branch) {
    const cacheKey = `${cwd}:${branch}`;
 ...
 */
export function clearPRForSession(sessionId, oldBranch, cwd) {
