 ...
 */
export async function tailJSONL(filepath, fromByte = 0) {
...
export function extractMetadata(entries) {
    ...
    for (const entry of entries) {
        ...
        if (cwd && sessionId && originalPrompt && startedAt)
            break;
    }
    if (!cwd || !sessionId || !startedAt)
        return null;
    return {
        sessionId,
        cwd,
 ...
 */
export function decodeProjectDir(encodedDir) {
 ...
 */
export function extractEncodedDir(filepath) {
