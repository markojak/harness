...
async function processAPITask(task) {
    try {
...
const apiQueue = fastq.promise(processAPITask, 1);
/**
 * Queue an API call and return the result
 */
function queueAPICall(params) {
 ...
 */
function generateContentHash(entries) {
 ...
 */
function extractContext(session) {
 ...
 */
export async function generateAISummary(session) {
 ...
 */
function getWorkingSummary(session) {
    ...
    if (lastAssistant && lastAssistant.type === "assistant") {
        ...
        if (tools.length > 0) {
            const tool = tools[0];
            const input = lastAssistant.message.content.find((b) => b.type === "tool_use")?.input;
 ...
 */
function getFallbackSummary(session) {
 ...
 */
export async function generateGoal(session) {
