...
export class AntigravityWatcher extends EventEmitter {
  private annotationWatcher: FSWatcher | null = null;
  private brainWatcher: FSWatcher | null = null;
  ...
  async start(): Promise<void> {
    ...
    await Promise.all([
      this.annotationWatcher ? new Promise<void>(resolve => this.annotationWatcher!.on("ready", resolve)) : Promise.resolve(),
      this.brainWatcher ? new Promise<void>(resolve => this.brainWatcher!.on("ready", resolve)) : Promise.resolve(),
      this.conversationWatcher ? new Promise<void>(resolve => this.conversationWatcher!.on("ready", resolve)) : Promise.resolve(),
    ]);
  ...
  private extractSessionIdFromPath(path: string, baseDir: string): string | null {
  ...
  private debouncedHandleAnnotation(path: string): void {
    const sessionId = basename(path, ".pbtxt");
    this.debouncedUpdateSession(sessionId);
  }

  private debouncedUpdateSession(sessionId: string): void {
  ...
  private async handleAnnotationChange(path: string): Promise<void> {
    const sessionId = basename(path, ".pbtxt");
    await this.updateSession(sessionId);
  }

  private async handleNewSession(sessionId: string): Promise<void> {
  ...
  private handleSessionDeleted(sessionId: string): void {
    ...
    if (session) {
      ...
      this.emit("session", {
        type: "deleted",
        session,
      } as AntigravitySessionEvent);
    }
  }

  private async updateSession(sessionId: string, isNew = false): Promise<void> {
    try {
      // Get annotation data
      ...
      let status: "active" | "idle" | "working" | "unknown" = "unknown";
      ...
      const session: AntigravitySessionState = {
        sessionId,
        filepath: convPath,
        projectName,
        projectPath,
