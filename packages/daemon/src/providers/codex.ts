 ...
 */
export async function parseCodexSession(filepath: string): Promise<ProviderSession | null> {
  try {
      ...
      try {
        ...
        if (parsed.type === "message") {
          ...
          if (parsed.role === "user" && !originalPrompt) {
            const textContent = parsed.content?.find((c: any) => 
              c.type === "input_text" || c.type === "text"
            );
            originalPrompt = textContent?.text || "";
          }
 ...
 */
export async function parseCodexEvents(filepath: string): Promise<SessionEvent[]> {
