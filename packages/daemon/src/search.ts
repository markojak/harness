...
export function indexSession(session: {
  ...
  upsert.run(
    ...
    session.projectId,
    session.projectName,
    session.gitRepoId || null,
    session.gitRepoUrl || null,
    session.cwd,
 ...
 */
export function getSearchStats(): { indexed: number; lastIndexed: string | null } {
 ...
 * Bulk index sessions
 */
export function bulkIndexSessions(sessions: Array<{
  sessionId: string;
  projectId: string;
  projectName: string;
  gitRepoId?: string | null;
