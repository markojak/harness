...
import { AntigravityWatcher, type AntigravitySessionEvent, type AntigravitySessionState } from "./antigravity-watcher.js";
import { StreamServer } from "./server.js";
import { formatStatus } from "./status.js";
import { startStatsServer, updateTodayCost } from "./system-stats.js";
import { aggregateUsage, type CostSummary } from "./cost-tracker.js";
 ...
 */
function isFromToday(session: SessionState): boolean {
 ...
 */
function calculateTodayCost(watcher: SessionWatcher): void {
...
async function main(): Promise<void> {
  ...
  if (isAntigravityAvailable()) {
    ...
    antigravityWatcher.on("session", async (event: AntigravitySessionEvent) => {
      const { type, session } = event;
      const timestamp = new Date().toLocaleTimeString();
