...
export async function extractSessionDetail(filepath: string): Promise<SessionDetail> {
      ...
      try {
        ...
        if (entry.type === "response_item" && entry.payload) {
          ...
          if (payload.role === "user") {
            ...
            const textContent = payload.content?.find((c: any) => 
        ...
        if (entry.type === "function_call" || (entry.type === "tool_use" && entry.name)) {
          ...
          if (args.file_path) target = args.file_path;
          else if (args.path) target = args.path;
          else if (args.command) target = String(args.command).slice(0, 50);
          else if (args.query) target = String(args.query).slice(0, 50);
          ...
          if (Array.isArray(blocks)) {
            for (const block of blocks) {
              ...
              if (block.type === "tool_use") {
                ...
                if (block.input) {
                  if (block.input.file_path) target = block.input.file_path;
                  else if (block.input.path) target = block.input.path;
                  else if (block.input.command) target = block.input.command.slice(0, 50);
                  else if (block.input.query) target = block.input.query.slice(0, 50);
                }
